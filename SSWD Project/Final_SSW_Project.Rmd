...
**COMPARING ALLELES IN INTERTIDAL**
```{r}
...
first10eig <- sum(16.061225, 14.986533, 14.719689, 14.539856, 14.465510, 13.910824, 13.490920, 13.197774, 12.867169, 12.351743)
...
var10 <- first10eig / totaleig #variation explained by first 10 PC bands
...
loadingplot(abs(intertidal.dapc$var.load), 
            lab.jitter=1, 
            threshold=quantile(abs(intertidal.dapc$var.load), probs=0.999))#threshold takes the top 0.1% of loci loading onto the DAPC that separates HH HS SS. This has the advantage of allowing one to quantify the contribution of a given allele to a particular structure.
...
#next: to see if this trend was not just related to this pop, take another ochraceus pop and perform the same analysis. do the same genes pop up in H and S? if so, perhaps they are related to (or linked to genes that are related to) pathogen resistance or susceptibility

```

**COMPARING ALLELES IN SUBTIDAL AND INTERTIDAL**
```{r}
...
Final project notes from steve and melissa:

30 min presentation: 20 minutes talk, 10 minute questions/discussion

equal time for each person in group

what's the library type that gives rise to 16s data
how were the reads cleaned and assembled

then for our analysis, more detailed. make clear the choices you made for your analysis. we chose to remove indivs or whatever

results and discussion: make sure you talk about the biology of the results FIRST. then back it up with statistical evidence

take home

talk about what this could be ...in discussion..then follow it up withthe next potential study. next questions? manipulative experiment?
